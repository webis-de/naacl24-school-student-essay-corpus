...
def experiment(level):
    print(level)
    if os.path.isfile(f'hyperparam-tuning-results/quality-mdeberta-adapter-{level}.csv'):
        results_list = list(pd.read_csv(f'hyperparam-tuning-results/quality-mdeberta-adapter-{level}.csv', index_col=0)[['fold']].itertuples(index=False, name=None))
        results = pd.read_csv(f'hyperparam-tuning-results/quality-mdeberta-adapter-{level}.csv', index_col=0).to_dict('records')
    else:
        results_list = []
        results = []        
    ...
    for fold in range(0, 10, 2):
        ...
        if not fold in results_list: # check if done already
            ...
            trainer = AdapterTrainer( #CustomTrainer
                model=model,                     
                args=training_args,              
            ...
            results_df.to_csv(f'hyperparam-tuning-results/quality-mdeberta-adapter-{level}.csv')                        
